...
func UnaryClientInterceptor() grpc.UnaryClientInterceptor {
	return requestid.UnaryClientInterceptor()
}

// UnaryServerInterceptor assigns a request ID and logs completed unary gRPC server calls.
func UnaryServerInterceptor() grpc.UnaryServerInterceptor {
	requestIDInterceptor := requestid.UnaryServerInterceptor()
	accessLogInterceptor := unaryServerInterceptor(loadConfig(), defaultLog)
	return func(ctx context.Context, req any, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (any, error) {
		return requestIDInterceptor(ctx, req, info, func(ctx context.Context, req any) (any, error) {
			return accessLogInterceptor(ctx, req, info, handler)
		})
	}
}
...
func unaryServerInterceptor(cfg config, log logFunc) grpc.UnaryServerInterceptor {
	...
	return func(ctx context.Context, req any, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (response any, err error) {
		...
		if requestID == "" {
			requestID = requestid.Ensure(requestid.FromIncomingMetadata(ctx))
